...
当前主集成路径是 Claude Code HTTP Hooks 和 Codex CLI command hooks。两者都通过 `vibe-island-hooks.exe` / HTTP Hook server 进入统一事件模型；Named Pipe SDK 继续保留为自定义 agent 的可选 fallback。Overlay 由透明、无边框、置顶的 Tauri WebView 窗口承载，Rust 侧提供 Win32 窗口样式、DPI、聚焦和进程监控能力。
...
```text
/
├── CLAUDE.md              # 项目配置和导航入口（AI Agent 入口）
├── WORKFLOW.md            # 工作流程和 OpenSpec 生命周期
├── architecture.md        # 本文件 - 架构约束 + 模块详情
├── openspec/              # 行为规格（specs/）+ 活跃 change（changes/）— 真相源
├── docs/history/          # 开发历史归档（原 progress.txt）
├── Task.json              # 已退役空壳（原任务列表，迁移到 openspec）
...
│       ├── codex_hook_config.rs   # Codex CLI hooks.json 自动配置/清理
...
│       ├── window_manager.rs      # 窗口管理工具
│       ├── terminal_jump/         # 终端跳转（workspace 检测、策略、解析）
│       ├── audio.rs               # 通知提示音播放
...
│       │   ├── AgentIcon.tsx      # Agent 品牌图标
│       │   ├── AnimatedNotch.tsx  # 动画 Notch 组件
...
3. Tauri setup 阶段：
   ...
   - Codex CLI hooks 通过 Control Center 或 tray 手动安装到 `~/.codex/hooks.json`；
...
Codex CLI 通过 command hooks 接入。Vibe Island 将 `vibe-island-hooks.exe --source codex` 写入 `~/.codex/hooks.json`，默认安装生命周期和轻量工具活动事件：`SessionStart(startup|resume|clear|compact)`、`UserPromptSubmit`、`PreToolUse(*)`、`PostToolUse(*)`、`Stop`。Hook helper 读取 Codex stdin payload，经 `\\.\pipe\VibeIsland` 转发给 `pipe_server.rs`，后端用 `CodexAdapter` 映射为统一 `AgentEvent`，前端显示 Codex 品牌、prompt 标题和工具活动。

默认不安装 `PermissionRequest`，因此 Codex 审批仍由 Codex 自身控制。完整配置见 `docs/hooks/codex-hooks-setup.md`。
...
> 行为真相见 `openspec/specs/state-machine/spec.md`（AgentState 枚举、TRANSITION_MATRIX、Hook 事件→状态映射）。本节为 UI 层 phase 概览。
...
> **行为真相源**：`openspec/specs/`。本节为高层概览与速查，详细 Requirements / Scenarios 见对应规格（`config-sync`、`hook-integration`、`state-machine`、`session-flow`、`frontend-error-handling`）。本节与规格若有冲突，以规格为准。
