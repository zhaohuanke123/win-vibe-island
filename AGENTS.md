...
Windows 桌面悬浮 Overlay 应用，监控 AI 编程助手会话（Codex、Codex、自定义 agent 等）并显示其状态。基于 Tauri 2.0（Rust + React）构建。
...
| Hook 配置、Codex 集成、hook 不工作、session_start 没触发 | hook-integration | Hook 配置和故障排查 |
...
- [Rust 编码规范](.Codex/rules/RustConvention.md)
- [React/TypeScript 编码规范](.Codex/rules/FrontendConvention.md)
- [Tauri IPC 规范](.Codex/rules/TauriIPCConvention.md)
- [架构约束](.Codex/rules/ArchitectureConstraints.md)
- [错误处理规范](.Codex/rules/ErrorHandlingConvention.md)
