...
- 前端使用 `ResizeObserver` 测量展开内容的自然高度，不能使用已被 flex/overflow 压缩后的单一 `panel.scrollHeight` 作为窗口高度来源。
- 总高度公式为：bar 52px + panel 垂直 padding + 内容自然高度 + footer 高度。footer 必须参与高度计算，设置按钮不能被外层 `overflow: hidden` 裁切。
- 实际同步给 Tauri 的高度必须限制在 180px 到 600px 之间。
- session 数量较少时窗口应贴合内容，不能保留大块空白；session 或审批内容较多时面板达到上限后，只由 session list / settings 内容区滚动，顶部 bar 和底部 footer 保持可见。
