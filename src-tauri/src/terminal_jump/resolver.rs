        ...
        match desc.focus_strategy {
            crate::terminal_jump::registry::FocusStrategyType::WindowsTerminal => {
                snapshot_windows_terminal()
                    ...
                    .map(|tab| (Some(tab.tab_id), Some(tab.tab_index), Some(tab.tab_id)))
...
pub fn detect_terminal_app(pid: u32) -> (Option<String>, Option<u32>) {
    ...
    let parent_pid = unsafe {
        ...
        if Process32FirstW(snapshot, &mut entry).is_ok() {
            ...
            loop {
                ...
                if Process32NextW(snapshot, &entry).is_err() {
                    break;
                }
            }
            found
    ...
    for _ in 0..10 {
        ...
        let exe_name = unsafe {
            ...
            if Process32FirstW(snapshot, &entry).is_ok() {
                let mut found = None;
                loop {
        ...
        if let Some(name) = exe_name {
            if let Some(desc) = crate::terminal_jump::registry::find_by_exe_name(&name) {
                ...
                let app = match desc.id {
                    "windows-terminal" => Some("WindowsTerminal".into()),
                    other => Some(desc.display_name.into()),
                };
                return (app, Some(current_pid));
            }
        }
